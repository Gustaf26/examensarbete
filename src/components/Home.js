...
const Home = () => {
  ...
  return (
    <>
      <div id="dummy-container-home" style={admin ? { position: 'absolute', left: mobile ? '40px' : '240px', width: mobile ? 'calc(100% - 40px)' : 'calc(100% - 240px)' } : {}} onClick={(e) => { if (e.target.id === "dummy-container-home") setMobileDisplays(false) }}>
        <Row id="home-row" onClick={(window.innerWidth < 1100 || mobile) && menuShowing ?
          ...
          lg={mobile ? 12 : 6}>
          {admin && <Navigation />}
          {mobile && admin && <Icon onClick={() => { alert(mobileDisplays); setMobileDisplays(!mobileDisplays) }} style={{
          ...
          <Card onClick={() => !admin && menuShowing && mobile ? setMenuShowing(false) : admin && mobile && mobileDisplays ? alert(mobileDisplays) : null} style={{
            padding: '10px',
            overflowY: mobile && admin ? 'scroll' : '', width: mobile ? '400px' : '600px',
            height: mobile && admin ? `calc(${mobileHeight - 80}px)` : 'fit-content',
